...
function saveLinkInfo($key_verse, $title, $language, $filename, $brand = 'bible-minutes') {
    if (empty($key_verse)) {
        return false;
    }
    
    // Parse the verse reference to get chapter info
    // Format: bookNo_chapter:verse or bookNo_chapter:verse-endVerse
    $parts = explode('_', $key_verse);
    if (count($parts) !== 2) {
        return false;
    }
    ...
    // Create verse-level links for each verse in the range
    for ($verse = $verseStart; $verse <= $verseEnd; $verse++) {
        ...
        foreach ($verse_links as $index => $link) {
            if (isset($link['brand']) && isset($link['filename']) &&
                $link['brand'] === $brand && $link['filename'] === $filename) {
    ...
    usort($chapter_links, function($a, $b) {
    ...
    // Save chapter-level file
    file_put_contents($chapter_file, json_encode($chapter_links, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    
    return true;
}

// Delete link information from verse-level and chapter-level indexes
function deleteLinkInfo($key_verse, $language, $filename, $brand = 'bible-minutes') {
    ...
    // Delete from each individual verse file in the range
    for ($verse = $verseStart; $verse <= $verseEnd; $verse++) {
        $individualVerseKey = "{$bookNo}_{$chapter}:{$verse}";
        $verse_file = $verses_dir . '/' . sanitizeFilename($individualVerseKey) . '.json';
        ...
        if (file_exists($verse_file)) {
            ...
            if (is_array($verse_links)) {
                // Remove the link matching this brand and filename
                $verse_links = array_filter($verse_links, function($link) use ($brand, $filename) {
                    // Check if keys exist before comparing
                    $linkBrand = $link['brand'] ?? '';
                    $linkFilename = $link['filename'] ?? '';
                    return !($linkBrand === $brand && $linkFilename === $filename);
                });
    ...
    if (file_exists($chapter_file)) {
        ...
        if (is_array($chapter_links)) {
            // Remove the meditation from the verse range entry
            foreach ($chapter_links as $idx => &$link) {
                if ($link['verse'] === $key_verse && isset($link['meditations']) && is_array($link['meditations'])) {
                    // Filter out the meditation matching this brand and filename
                    ...
                    // If no meditations left for this verse range, remove the verse entry
...
/**
 * Sanitize filename by replacing special characters with hyphens
 */
...
if ($is_logged_in || true) {
    ...
    if ($action === 'add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Save link information - brand is always 'bible-minutes' for this admin panel
        saveLinkInfo($_POST['key_verse'], $_POST['title'], $language, $filename, 'bible-minutes');
    ...
    if ($action === 'delete') {
        ...
        if (file_exists($file)) {
            // Read meditation data to get key verse before deleting
            $meditation_data = json_decode(file_get_contents($file), true);
            
            // Delete the meditation file
            unlink($file);
            updateAllMeditationsFile($language);
            
            // Delete link information if key verse exists
            if (!empty($meditation_data['key_verse'])) {
                deleteLinkInfo($meditation_data['key_verse'], $language, $filename, 'bible-minutes');
            }
...
<head>
    ...
    <title>Admin Panel - Bible Minutes</title>
    ...
    <meta name="apple-mobile-web-app-title" content="Admin - Bible Minutes">
    ...
    <nav class="navbar navbar-expand-lg navbar-dark admin-navbar">
        <div class="container">
            <a class="navbar-brand fw-bold" href="../index.php">
                <i class="bi bi-shield-lock me-2"></i>Admin Panel - Bible Minutes
            </a>
                                    ...
                                    <div class="col-md-6">
                                        <div class="admin-form-group">
                                            ...
                                            <input type="text" class="admin-form-control" id="author_name" name="author_name" 
                                                   value="<?php echo $edit_meditation ? htmlspecialchars($edit_meditation['author']['author']) : 'Yesudas Solomon'; ?>" required>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="admin-form-group">
