...
function getAppName($language) {
    ...
    return "நாளுக்கொரு நல்ல பங்கு"; // Default fallback
}
...
function getCurrentMeditationIndex($mode, $action, $uniqueid, $language) {
    ...
    if ($mode === 'random') {
        ...
        // If a specific uniqueid is requested, find its index
        if ($uniqueid !== null) {
            $targetIndex = findMeditationIndexByUniqueId($uniqueid, $allMeditations);
            // Validate the target index exists
            if ($targetIndex !== null) {
                ...
                // Uniqueid not found, reset to first
                $_SESSION['random_index'] = 0;
            }
        }
...
<body>
    ...
    <div class="site-header">
        <div class="container">
            <h1><i class="bi bi-calendar-heart"></i> <?php echo htmlspecialchars($appName); ?></h1>
                        ...
                        <div class="section">
                            ...
                            <?php if (!empty($meditation['author']['whatsapp'])): ?>
                                <p class="mb-1">
                                    ...
                                    <a href="https://wa.me/<?php echo preg_replace('/[^0-9]/', '', $meditation['author']['whatsapp']); ?>?text=நாளுக்கொரு-நல்ல-பங்கு" 
